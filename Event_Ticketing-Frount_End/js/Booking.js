...
function bookNow() {
    ...
    var seatIds = seatIdsRaw.split(',').map(function(s){ return parseInt(s.trim()); }).filter(function(n){ return !isNaN(n) && n > 0; });
    ...
    $.ajax({
        ...
        success: function(res) {
            ...
            Swal.fire({
                ...
                html:               '<p>Booking <strong>#' + booking.booking_id + '</strong> is confirmed.</p>' +
        ...
        success: function(res) {
            ...
            bookings.forEach(function(b) {
                ...
                var row =
                    ...
                    ' data-userid="' + b.user_id      + '"' +
                    ' data-eventid="'+ b.eventId     + '"' +
                    ' data-amount="' + b.total_amount + '"' +
                    ' data-coupon="' + (b.couponCode||'') + '"' +
                    ' data-status="' + b.status       + '"' +
                    ' data-seats="'  + (b.seat_ids||[]).join(',') + '"' +
                    ' style="cursor:pointer;">' +
                    '<td class="muted">#' + b.booking_id + '</td>' +
                    '<td class="muted">' + b.user_id + '</td>' +
                    '<td><strong>' + (b.event_name || b.eventId) + '</strong></td>' +
                    '<td class="muted">' + formatDate(b.booking_date) + '</td>' +
                    '<td>LKR ' + Number(b.total_amount).toLocaleString() + '</td>' +
...
function deleteBookingRow(btn) {
    ...
    Swal.fire({
        ...
        $.ajax({
            ...
            success: function() { SWAL.success('Cancelled', 'Booking #' + id + ' cancelled and seats released.'); getAllBookings(); },
            error:   function(err) {
